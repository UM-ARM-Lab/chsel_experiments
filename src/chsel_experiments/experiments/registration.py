...
from chsel_experiments.registration import methods, volumetric
from base_experiments.env.pybullet_env import make_sphere
from chsel_experiments.experiments.registration_nopytorch3d import saved_traj_dir_base, saved_traj_dir_for_method
from chsel import costs as icp_costs
from chsel_experiments.registration.medial_constraints import MedialBall
...
def do_registration(model_points_world_frame, model_points_register, best_tsf_guess, B,
                    volumetric_cost: icp_costs.VolumetricCost, reg_method: registration.ICPMethod, **kwargs):
    ...
    if reg_method == registration.ICPMethod.ICP:
    ...
    elif reg_method == registration.ICPMethod.ICP_SGD_REVERSE:
    ...
    # use only volumetric loss
    elif reg_method == registration.ICPMethod.ICP_SGD_VOLUMETRIC_NO_ALIGNMENT:
        T, distances = methods.icp_pytorch3d_sgd(model_points_world_frame, model_points_register,
                                                 given_init_pose=best_tsf_guess.inverse(), batch=B,
    ...
    elif reg_method in [registration.ICPMethod.VOLUMETRIC, registration.ICPMethod.VOLUMETRIC_NO_FREESPACE,
                        registration.ICPMethod.VOLUMETRIC_ICP_INIT, registration.ICPMethod.VOLUMETRIC_LIMITED_REINIT,
                        registration.ICPMethod.VOLUMETRIC_LIMITED_REINIT_FULL,
                        registration.ICPMethod.VOLUMETRIC_CMAES, registration.ICPMethod.VOLUMETRIC_CMAME,
                        registration.ICPMethod.VOLUMETRIC_CMAMEGA,
                        registration.ICPMethod.VOLUMETRIC_SVGD]:
        if reg_method == registration.ICPMethod.VOLUMETRIC_NO_FREESPACE:
            volumetric_cost = copy.copy(volumetric_cost)
            volumetric_cost.scale_known_freespace = 0
        if reg_method == registration.ICPMethod.VOLUMETRIC_ICP_INIT:
        ...
        if reg_method == registration.ICPMethod.VOLUMETRIC_CMAES:
            optimization = volumetric.Optimization.CMAES
        elif reg_method == registration.ICPMethod.VOLUMETRIC_CMAME:
            optimization = volumetric.Optimization.CMAME
        elif reg_method == registration.ICPMethod.VOLUMETRIC_CMAMEGA:
        ...
        T, distances = methods.volumetric_registration(volumetric_cost, model_points_world_frame, optimization=optimization,
    ...
    # T, distances = registration.icp_stein(model_points_world_frame, model_points_register, given_init_pose=T.inverse(),
    #                              batch=B)
    return T, distances
...
def do_medial_constraint_registration(reg_method, model_points_world_frame, obj_sdf: ObjectFrameSDF, best_tsf_guess, B,
    ...
    balls_path = f"{saved_traj_dir_for_method(registration.ICPMethod.MEDIAL_CONSTRAINT, experiment_name=experiment_name)}/{level.name}_{seed}_balls.txt"
    ...
    T, distances = methods.icp_medial_constraints(obj_sdf, balls,
                                                  ...
                                                  cmame=reg_method == registration.ICPMethod.MEDIAL_CONSTRAINT_CMAME,
